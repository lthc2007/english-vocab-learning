...
### 🎤 发音测评（讯飞）
- 入口：单词详情弹窗（单词发音测评 / 例句发音测评）+ Learn 卡片（发音测评）+ 文章陪读（每段发音测评）
- 点击后可选择「直接发音测评」或「先听标准音（TTS）再发音」
- 评分反馈：总分 + 准确度/流利度/完整度 + 逐词红绿着色（漏读/增读/回读/替换标注）
- 总分低于 60 分提示「建议重读」；超时/网络错误自动重试一次
- 录音先缓存到本地：评测成功自动清除，失败可直接用刚才的录音重传，无需重新录制
- Learn 卡片中已测评过的单词，再次进入先展示上次测评结果，可一键重新测评
